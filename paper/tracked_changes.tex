...
%DIF DEL paper_original_submission.tex   Sun Mar  6 07:13:18 2022
%DIF ADD paper.tex                       Mon Mar  7 15:07:03 2022
...
\title{An antibody-escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend for mutations to the SARS-CoV-2 receptor-binding domain}
...
%DIF 51-52c51-52
%DIF < Here we help address this challenge by aggregating deep mutational scanning data into an ``escape calculator'' that estimates the antigenic effects of arbitrary combinations of mutations to the virus's spike receptor-binding domain (RBD).
%DIF < The calculator can be used to intuitively visualize how mutations impact polyclonal antibody recognition, and score the expected antigenic effect of combinations of mutations.
%DIF -------
Here we help address this challenge by aggregating deep mutational scanning data into an ``escape estimator'' that estimates the antigenic effects of arbitrary combinations of mutations to the virus's spike receptor-binding domain (RBD). %DIF > 
The estimator can be used to intuitively visualize how mutations impact polyclonal antibody recognition, and score the expected antigenic effect of combinations of mutations. %DIF > 
%DIF -------
...
%DIF 54c54-55
%DIF < An interactive version of the calculator is at \url{https://jbloomlab.github.io/SARS2_RBD_Ab_escape_maps/escape-calc/}, and we provide a Python module for batch processing.
%DIF -------
An interactive version of the estimator is at \url{https://jbloomlab.github.io/SARS2_RBD_Ab_escape_maps/escape-calc/}, and we provide a Python module for batch processing. %DIF > 
Currently the calculator uses primarily data for antibodies elicited by Wuhan-Hu-1-like vaccination or infection, and so is expected to work best for calculating escape from such immunity for mutations relative to early SARS-CoV-2 strains. %DIF > 
%DIF -------
\end{abstract}
...
This antigenic evolution is already apparent for SARS-CoV-2, as new viral variants with reduced antibody neutralization \DIFdelbegin \DIFdel{have emerged only $\sim$2 years since }\DIFdelend \DIFaddbegin \DIFadd{emerged within a year of when }\DIFaddend the virus first started to spread in humans.
...
However, SARS-CoV-2 variants of concern \DIFaddbegin \DIFadd{(variants with reduced immune recognition, enhanced transmissibility, or increased virulence) }\DIFaddend typically have multiple mutations, and it is not feasible to experimentally characterize all combinations of mutations even via high-throughput approaches such as deep mutational scanning.
...
The resulting ``escape \DIFdelbegin \DIFdel{calculator}\DIFdelend \DIFaddbegin \DIFadd{estimator}\DIFaddend '' enables qualitative visualization and quantitative scoring of the antigenic effects of arbitrary combinations of mutations.
Importantly, the escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend is based on simple transformations of direct experimental measurements, and \DIFdelbegin \DIFdel{does not involve complex black-box computational methods}\DIFdelend \DIFaddbegin \DIFadd{and so its calculations can be intuitively visualized using the interactive interface we provide}\DIFaddend . 

\section{Results}

\subsection{Combining monoclonal antibody escape maps reveals correlated and independent viral antigenic mutations}
A deep mutational scanning experiment can measure how all single amino-acid mutations to the SARS-CoV-2 RBD affect binding by a monoclonal antibody~\citep{greaney2021complete}\DIFaddbegin \DIFadd{, where we here consider mutations at the level of amino-acid replacements rather than in terms of the underlying nucleotide changes}\DIFaddend .
...
Now \DIFdelbegin \DIFdel{imagine a }\DIFdelend \DIFaddbegin \DIFadd{consider a hypothetical }\DIFaddend polyclonal antibody mix of these three antibodies combined at equal potencies.
...
This score is defined using the same principle as the site-wise escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend in the previous section: we reduce the contribution of each antibody by an amount that scales with how strongly it is escaped by each mutated site, and define the overall score as the fraction of all antibody contributions that remain (see Methods for details).
This calculation is implemented in the interactive \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend at \url{https://jbloomlab.github.io/SARS2_RBD_Ab_escape_maps/escape-calc/}, and returns a score that ranges from one (no mutations affect binding of any antibodies) to zero (all antibodies fully escaped).
...
\DIFaddbegin \DIFadd{Labels at the top of each plot shown the Pearson's R and associated P-value.
}\DIFaddend An interactive version of this figure that allows mousing over points to see details is at \url{https://jbloomlab.github.io/RBD_escape_calculator_paper/neut_studies.html}.
\DIFaddbegin \DIFadd{Supplementary Figure S1 shows almost identical results are obtained if the binding scores are computed using the mean of mutations at a site rather than the sum.
}\DIFaddend }
\label{fig:neut_studies}
\end{SCfigure*}

To test how these \DIFdelbegin \DIFdel{escape-calculator }\DIFdelend \DIFaddbegin \DIFadd{escape-estimator }\DIFaddend scores compare to experimentally measured neutralization titers, we collated neutralization data from three previously published studies~\citep{lucas2021impact,uriu2021neutralization,wang2021antibody}, each of which characterized sera from two patient cohorts against a variety of SARS-CoV-2 variants and mutants.
One can imagine many reasons why the \DIFdelbegin \DIFdel{escape-calculator }\DIFdelend \DIFaddbegin \DIFadd{escape-estimator }\DIFaddend scores might differ from the real neutralization titers: the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend only considers RBD mutations, the antibodies used by the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend might not accurately reflect the real mix in polyclonal sera, etc.
But despite all these potential caveats, the \DIFdelbegin \DIFdel{escape-calculator }\DIFdelend \DIFaddbegin \DIFadd{escape-estimator }\DIFaddend scores correlate quite well with the measured neutralization titers across all studies and cohorts (Figure~\ref{fig:neut_studies}).
Therefore, the simple and intuitive approach used by the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend seems to accurately reflect the dominant features of polyclonal antibody escape in the RBD.

\subsection{The escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend suggests extensive antigenic change in the new Omicron variant}
We applied the escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend to the recently reported Omicron variant, which has 15 mutated sites in its RBD~\citep{ngs-sa2021update,deoliveira2021tweet}.
...
Another key aspect of our approach is the interactive visual implementation of the escape \DIFdelbegin \DIFdel{calculator}\DIFdelend \DIFaddbegin \DIFadd{estimator}\DIFaddend , so the user can interrogate the effects of mutations (or combinations of mutations) simply by clicking (or shift-clicking) on sites.
This interactivity provides an intuitive understanding of the antigenic structure of the RBD, and shows how the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend is performing simple transformations directly on experimental data.
We encourage interactive use of the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend so it acts as \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a visual }\DIFaddend aid to augment human interpretation\DIFdelbegin \DIFdel{, rather than a black-box computational method}\DIFdelend .
However, we note that in addition to the interactive \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend at \url{https://jbloomlab.github.io/SARS2_RBD_Ab_escape_maps/escape-calc/}, we also provide a Python module for batch processing of large numbers of sequences at \url{https://github.com/jbloomlab/SARS2_RBD_Ab_escape_maps/}.

There are caveats that should be kept in mind when using the escape \DIFdelbegin \DIFdel{calculator}\DIFdelend \DIFaddbegin \DIFadd{estimator}\DIFaddend .
First, the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend only considers sites in the RBD, and ignores mutations to other regions of spike.
Second, the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend assumes the neutralizing activity of human polyclonal serum is represented by an equipotent mix of the monoclonal antibodies that happen to have been previously characterized by deep mutational scanning.
Third, the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend simply averages site-level escape measurements across antibodies, and \DIFaddbegin \DIFadd{ignores differences in the effects of different amino-acid mutations at the same site.
Fourth, the estimator }\DIFaddend does not yet implement a real biophysical model of the combined activity of multiple antibodies~\citep{einav2020two}.
Finally, and in our minds most significantly, the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend estimates the impact of mutations in reference to antibodies targeted to the early Wuhan-Hu-1 RBD---an approach that is currently reasonable, but will become problematic as human exposure and vaccination histories diversify in the years to come (see last paragraph).

Despite all these caveats, the escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend yields binding scores that correlate with experimentally measured neutralization titers.
In addition, the actual antigenic evolution of SARS-CoV-2 seems to follow the principles captured by the escape \DIFdelbegin \DIFdel{calculator}\DIFdelend \DIFaddbegin \DIFadd{estimator}\DIFaddend : variants of concern generally have combinations of mutations calculated to have additive effects on antibody escape (e.g., 417 and 484) rather than combinations calculated to have redundant effects (e.g., 484 and 490).
We suspect the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend works well because the RBD is the dominant target of neutralizing activity~\citep{piccoli2020mapping,greaney2021comprehensive,schmidt2021high} and the human antibody-response to the early Wuhan-Hu-1 RBD shares broad commonalities across individuals~\citep{yuan2020structural,robbiani2020convergent,greaney2021comprehensive,greaney2021mapping,chen2021convergent}.
...
So in the future, it will be necessary to stratify the data used by the escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend by which RBD variant elicited the antibodies, and aggregate data for antibodies that reflect the sera in question.
For this reason, we expect to continue adding to the data used by the escape \DIFdelbegin \DIFdel{calculator}\DIFdelend \DIFaddbegin \DIFadd{estimator}\DIFaddend , and emphasize that it will change over time from the version described here, although we provide stable links to the current version in the Methods below. 

{\small

\section{Methods}
\subsection{Code and data availability}
The most up-to-date code and data used to implement the escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend are at \url{https://github.com/jbloomlab/SARS2_RBD_Ab_escape_maps}, and the version described in this paper are at \url{https://github.com/jbloomlab/SARS2_RBD_Ab_escape_maps/tree/bioRxiv_v1}.

The data used by the escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend are at \url{https://github.com/jbloomlab/SARS2_RBD_Ab_escape_maps/blob/main/processed_data/escape_calculator_data.csv}, and the version used for this paper are at \url{https://github.com/jbloomlab/SARS2_RBD_Ab_escape_maps/blob/bioRxiv_v1/processed_data/escape_calculator_data.csv}.

A downloadable HTML version of the escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend is at \url{https://github.com/jbloomlab/SARS2_RBD_Ab_escape_maps/raw/main/docs/_includes/escape_calc_chart.html}, with the version described in this paper at \url{https://github.com/jbloomlab/SARS2_RBD_Ab_escape_maps/blob/bioRxiv_v1/docs/_includes/escape_calc_chart.html}.
...
\subsection{Deep mutational scanning data used by the \DIFdelbegin \DIFdel{calculator}\DIFdelend \DIFaddbegin \DIFadd{estimator}\DIFaddend }
The experimental data used by the escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend are drawn from seven previously published deep mutational scanning studies~\citep{greaney2021complete,greaney2021mapping,starr2021prospective,starr2021complete,starr2021sars,dong2021genetic,tortorici2021broad} and one unpublished dataset available at \url{https://github.com/jbloomlab/SARS-CoV-2-RBD_MAP_COV2-2955}.
In total, these studies contain data for 36 monoclonal antibodies.
Three of these antibodies (CR3022, S304, and S309) were elicited by infection with SARS-CoV-1 and so are excluded from the datasets used for the calculations in this paper, although the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend has an option (\texttt{eliciting\_virus}) that allows optional inclusion of these antibodies.
...
The results reported in this paper use the sums as the site-level metric, although the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend has an option (\texttt{escape\_metric}) to use the mean instead \DIFdelbegin \DIFdel{.
We also }\DIFdelend \DIFaddbegin \DIFadd{(the results are almost identical: compare Fig~\ref{fig:neut_studies} versus Supplementary Figure S1.
}

\DIFadd{We }\DIFaddend normalize the site-level escape metrics for each antibody to account for different strengths of antibody selection in different experiments using the approach described in \citet{greaney2021comprehensive} and implemented in \url{https://jbloomlab.github.io/dmslogo/dmslogo.utils.html#dmslogo.utils.AxLimSetter} with \texttt{min\_upperlim=1} and \texttt{max\_from\_quantile=(0.5, 0.05)}: essentially this corresponds to scaling the site escape values for each antibody so that a value of one corresponds to the larger of the maximum escape at a site or 20 times the median value across sites.
\DIFdelbegin \DIFdel{The results reported in this paper use the normalized }\DIFdelend \DIFaddbegin \DIFadd{Specifically, the normalization is done so that for each antibody $a$, the maximum escape $x_{a,r}$ at any site $r$ is set so that $1 = \max\left(\max_r \left[x_{a,r}\right], 20\tilde{x}_{a,r} \right)$ where $\tilde{x}_{a,r}$ is the median of $x_{a,r}$ across sites $r$ for antibody $a$.
The rationale for this normalization is that it usually scales the }\DIFaddend site-level \DIFdelbegin \DIFdel{metrics, although the calculator has an option (}\texttt{\DIFdel{escape\_values\_normalized}}%DIFAUXCMD
\DIFdel{) to also use the non-normalized values}\DIFdelend \DIFaddbegin \DIFadd{escape metric so that the maximum value at a site for each antibody is one, but for very ``flat'' escape profiles where no site has more escape than 20 times the median, then the maximum value is smaller corresponding to no real peaks of escape for the antibody}\DIFaddend .


\subsection{\DIFdelbegin \DIFdel{Calculation }\DIFdelend \DIFaddbegin \DIFadd{Estimation }\DIFaddend of the impact of mutations}
The escape \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend determines the impact of mutating sites by calculating how much each antibody is escaped by mutations at each site, and adjusting its contribution to the overall polyclonal mix accordingly.
...
In this paper we report calculations with $s = 2$, although the \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend has an option (\texttt{mutation\_escape\_strength}) to choose other values.
We then define the escape map after the mutations $\mathcal{M}$ as $\frac{1}{A}\sum_a x_{a,r} b_a\left(\mathcal{M}\right)$.
The \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend shows the escape map with no mutations in gray, and that after mutations in blue.
...
\subsection{Implementation of interactive \DIFdelbegin \DIFdel{calculator}\DIFdelend \DIFaddbegin \DIFadd{estimator}\DIFaddend }
The interactive \DIFdelbegin \DIFdel{calculator }\DIFdelend \DIFaddbegin \DIFadd{estimator }\DIFaddend at \url{https://jbloomlab.github.io/SARS2_RBD_Ab_escape_maps/escape-calc/} is imlemented using Altair (\url{https://altair-viz.github.io/})~\citep{VanderPlas2018}, which is in turn built upon Vega-Lite~\citep{Satyanarayan2017}.

\subsection{Python module with batch-mode \DIFdelbegin \DIFdel{calculator}\DIFdelend \DIFaddbegin \DIFadd{estimator}\DIFaddend }
A Python module that implements the calculations is at \url{https://github.com/jbloomlab/SARS2_RBD_Ab_escape_maps/}, and has all the same options as the interactive \DIFdelbegin \DIFdel{calculator}\DIFdelend \DIFaddbegin \DIFadd{estimator}\DIFaddend .
...
This work was supported in part by the NIH/NIAID under \DIFaddbegin \DIFadd{contract HHS75N93021C00015, and grants }\DIFaddend R01AI141707 (to JDB) and T32AI083203 (to AJG).
